...
pub fn compile(cwd: &PathBuf, ffi_code: impl ToString, input: impl ToString, target: impl Target) -> Result<()> {
    match parse(input).compile(cwd) {
        Ok(mir) => match mir.assemble() {
            Ok(asm) => match asm.assemble(&target) {
                ...
                Ok(result) => target.compile(target.prelude() + &ffi_code.to_string() + &result + &target.postlude()),
                Err(e) => {
